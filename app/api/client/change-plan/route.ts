...
<table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border:1px solid #e0e0e0;max-width:560px;width:100%;">
  ...
  <tr><td style="padding:32px;">
    <table cellpadding="0" cellspacing="0" style="margin:0 0 20px;">
      <tr><td style="background:#C9A84C;color:#0A0A0A;font-weight:700;padding:5px 14px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;">
        ${isUpgrade ? `${planLabel} Plan Active` : `Switching to ${planLabel}`}
      </td></tr>
    </table>
    ...
    <p style="font-size:13px;color:#888888;margin:0;line-height:1.6;">
      Questions? Simply reply to this email — we typically respond within a few hours.
    </p>
  </td></tr>
  <tr><td style="padding:16px 32px;background:#F9F9F9;border-top:1px solid #E0E0E0;">
    ...
    console.log(`Plan change email sent to ${email} (${newPlan}, ${isUpgrade ? 'upgrade' : 'downgrade'})`);
  } catch (err) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isUpgrade) {
      ...
      await supabase
        .from('clients')
        .update({ plan: targetPlan, pending_plan_change: null })
        .eq('id', client.id);
      ...
      // Period dates are on the subscription item in this API version
      const periodStart = subscriptionItem.current_period_start;
      const periodEnd = subscriptionItem.current_period_end;

      // Use Stripe Subscription Schedules to schedule the change
      ...
      await stripe.subscriptionSchedules.update(schedule.id, {
        ...
        phases: [
          {
            items: [{ price: currentPlan.priceId }],
      ...
      // Store pending change so dashboard can display it
